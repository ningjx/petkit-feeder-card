...
`;async function Ie(e,t,i="Button"){try{await e.callService("button","press",{entity_id:t})}catch(e){console.error(`[PetkitFeeder] ${i}失败:`,e)}}class Te{_editingItem=null;_originalItemData=null;get editingItem(){return this._editingItem}get originalItemData(){return this._originalItemData}get isEditing(){return null!==this._editingItem}startEdit(e,t,i,s,n,a,o,r){this._editingItem={itemId:e,field:t,time:i,name:s,amount:n},this._originalItemData={time:a,name:o,amount:r}}updateEditData(e,t,i){this._editingItem&&(void 0!==e&&(this._editingItem.time=e),void 0!==t&&(this._editingItem.name=t),void 0!==i&&(this._editingItem.amount=i))}cancelEdit(){this._editingItem=null,this._originalItemData=null}completeEdit(){this._editingItem=null,this._originalItemData=null}}function Pe(e,t){const i=new Map,s=new Map;e.filter(e=>"plan"===e.itemType).forEach(e=>{i.set(e.itemId,e)}),t.filter(e=>"plan"===e.itemType).forEach(e=>{s.set(e.itemId,e)});const n={added:[],modified:[],deleted:[]};return s.forEach((e,t)=>{const s=i.get(t);s?function(e,t){return e.time!==t.time||e.name!==t.name||e.plannedAmount!==t.plannedAmount||e.isEnabled!==t.isEnabled}(s,e)&&n.modified.push(e):n.added.push(e)}),i.forEach((e,t)=>{s.has(t)||n.deleted.push(t)}),0===n.added.length&&0===n.modified.length&&0===n.deleted.length?null:n}class ze{_originData=null;_cacheData=null;get originData(){return this._originData}initOrigin(e){this._originData=this._deepClone(e),this._cacheData=this._deepClone(e)}getDayCache(e){return this._cacheData?.days.get(e)||null}rollback(){this._originData&&(this._cacheData=this._deepClone(this._originData))}commit(){this._cacheData&&(this._originData=this._deepClone(this._cacheData))}detectChanges(){if(!this._originData||!this._cacheData)return[];const e=[];for(let t=1;t<=7;t++){const i=this._originData.days.get(t),s=this._cacheData.days.get(t);if(!i||!s)continue;Pe(i.timeline,s.timeline)&&e.push({day:t,items:s.timeline.filter(e=>"plan"===e.itemType).map(e=>({time:e.time,amount:e.plannedAmount,name:e.name,enabled:e.isEnabled}))})}return e}_deepClone(e){const t=new Map;return e.days.forEach((e,i)=>{t.set(i,{...e,timeline:this._deepCloneTimeline(e.timeline),summary:{...e.summary}})}),{days:t,lastUpdated:e.lastUpdated}}_deepCloneTimeline(e){return e.map(e=>({...e}))}}let Ue=class extends le{hass;_config;_weeklyCache=new ze;_editState=new Te;_selectedDay=1;_saveDebounceTimer=null;_isSaving=!1;_pendingFocus=null;_isInEditMode=!1;_isTogglingItem=!1;_getEntityId(e){return this._config?function(e,t){const i=e.device_id;return i?`sensor.petkit_feeder_${i}_${t}`:e.entity||""}(this._config,e):""}static getStubConfig(){return{device_id:"",show_timeline:!0,show_summary:!0,show_actions:!0}}static getConfigElement(){return document.createElement("petkit-feeder-card-editor")}setConfig(e){if(!e.device_id&&!e.entity)throw new Error($e("error.device_or_entity_required"));this._config={...e,show_timeline:e.show_timeline??!0,show_summary:e.show_summary??!0,show_actions:e.show_actions??!0}}shouldUpdate(){return!0}updated(){if(this._pendingFocus){const{itemId:e,field:t}=this._pendingFocus,i=`edit-${t}-${e}`,s=this.shadowRoot?.getElementById(i);s&&(s.focus(),"name"===t?s.select():"time"===t&&s.showPicker?.()),this._isInEditMode=!1,this._pendingFocus=null}}render(){if(!this._config||!this.hass)return B`<div>${this._localize("common.loading")}</div>`;const e=this._config.entity||this._getEntityId("feeding_schedule"),t=this._config.history_entity||this._getEntityId("feeding_records"),i=this.hass.states[e],s=t?this.hass.states[t]:null;if(!i)return B`
        <ha-card>
      ...
      `;const n=(a=this._config).connectivity_entity||`binary_sensor.petkit_feeder_${a.device_id}_online`;var a;const o=this.hass.states[n],r="on"===o?.state,d=we(this.hass),l=Ee(i.attributes,s?.attributes||{},r,d);this._weeklyCache.originData||(this._weeklyCache.initOrigin(l),this._selectedDay=_e());const c=this._weeklyCache.getDayCache(this._selectedDay),h=c?.timeline||[],u=c?.summary||{planAmount:0,actualAmount:0,manualAmount:0,isOnline:!1,totalCount:0,completedCount:0,pendingCount:0};let m=this._config.name;if(!m){const e=this._config.device_name_entity||this._getEntityId("device_name"),t=this.hass.states[e];m=t?.state}return m||(m=i.attributes.friendly_name||this._localize("default.device_name")),B`
      <ha-card @focusout=${this._handleCardFocusOut}>
        <div class="header">
          <span class="header-title">${m}</span>
          <span class="header-date">${function(e,t){const i=new Date,s=i.getDay(),n=e-(0===s?7:s),a=new Date(i);a.setDate(i.getDate()+n);const o=a.getMonth()+1,r=a.getDate(),d=t(`weekday.${e}`);return t("date.format",{month:o,day:r,weekday:d})}(this._selectedDay,this._localize.bind(this))}</span>
        ...
        ${this._config.show_timeline?this._renderTimeline(h):""}
        ${this._config.show_timeline?this._renderAddPlanButton():""}
        ${this._config.show_summary?this._renderSummary(u):""}
      </ha-card>
    ...
    `}_handleDaySwitch(e){this._selectedDay!==e&&(this._selectedDay=e,this._cancelEdit(),this.requestUpdate())}_renderTimeline(e){return e.length?B`
      <div class="section">
    ...
    `:B`
      ...
      `}_renderTimelineItem(e){const t=this._editState.editingItem,i=t?.itemId===e.itemId?t?.field:null,s=function(e,t){const i=_e();if(t<i)return!0;if(t!==i||!e.time)return!1;const s=new Date,n=60*s.getHours()+s.getMinutes(),[a,o]=e.time.split(":").map(Number);return 60*a+o<n}(e,this._selectedDay),n="plan"===e.itemType&&e.canDisable&&!s,a="plan"===e.itemType&&e.canDelete,o="plan"===e.itemType,r="time"===i&&t?B`
          <input
          ...
          />
        `:B`<span class="time ${o?"editable":""}" @click=${o?()=>this._startEdit(e,"time"):void 0}>${e.time}</span>`,d="name"===i&&t?B`
          <input
          ...
          />
        `:B`<span class="name ${o?"editable":""}" @click=${o?()=>this._startEdit(e,"name"):void 0}>${e.name}</span>`,l=void 0!==e.actualAmount?e.actualAmount:e.plannedAmount,c="amount"===i&&t?B`
          <input
            ...
            @keydown=${e=>{"Escape"===e.key&&this._cancelEdit()}}
          />
        `:B`<span class="amount ${o?"editable":""}" @click=${o?()=>this._startEdit(e,"amount"):void 0}>${l}g</span>`;return B`
      <div class="timeline-item ${e.itemType} ${i?"editing":""} ${"deleted_plan"===e.itemType?"plan-deleted":""}">
        <div class="item-row">
          ${r}
          ${d}
          ${c}
  ...
  `})(e.isExecuted)}
          <div class="item-actions">
            ${this._config?.show_actions?B`
                  <div
                    class="toggle-switch ${e.isEnabled?"on":"off"} ${n?"":"disabled"}"
                    @click=${n?()=>this._handleToggle(e):void 0}
                    title="${function(e,t,i){return"deleted_plan"===e.itemType?i("status.deleted"):t?i("status.expired"):e.isExecuted?i("status.executed"):e.isEnabled?i("status.click_disable"):i("status.click_enable")}(e,s,this._localize.bind(this))}"
                  >
                    <div class="toggle-thumb"></div>
                  </div>
                  <button
                    class="icon-delete-btn ${a?"":"disabled"}"
                    @click=${a?()=>this._handleDelete(e):void 0}
...
`)();_localize(e,t){return $e(e,we(this.hass),t)}};e([pe({attribute:!1})],Ue.prototype,"hass",void 0),e([pe({attribute:!1})],Ue.prototype,"_config",void 0),Ue=e([he("petkit-feeder-card")],Ue),customElements.get("petkit-feeder-card")||customElements.define("petkit-feeder-card",Ue);let Me=class extends le{hass;config;_schema=[{name:"device_id",required:!1,selector:{text:{}},label:"editor.device_id"},{name:"entity",required:!1,selector:{entity:{domain:["sensor"]}},label:"editor.entity"},{name:"history_entity",required:!1,selector:{entity:{domain:["sensor"]}},label:"editor.history_entity"},{name:"name",selector:{text:{}},label:"editor.card_title"},{type:"grid",name:"",title:"editor.display_control",schema:[{name:"show_timeline",selector:{boolean:{}},label:"editor.show_timeline"},{name:"show_summary",selector:{boolean:{}},label:"editor.show_summary"},{name:"show_actions",selector:{boolean:{}},label:"editor.show_actions"}]}];render(){return this.hass&&this.config?B`
      ...
      ></ha-form>
    `:B`<div>${this._localize("common.loading_editor")}</div>`}_computeLabel=e=>this._localize(e.name||e.label||e.title);_valueChanged(e){const t=e.detail.value;this.config=t;const i=new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0});this.dispatchEvent(i)}_localize(e){return $e(e,we(this.hass||{}))}static get styles(){return o`
      :host {
        display: block;
      }
      ha-form {
    ...
    `}};e([pe()],Me.prototype,"hass",void 0),e([pe()],Me.prototype,"config",void 0),Me=e([he("petkit-feeder-card-editor")],Me),customElements.get("petkit-feeder-card-editor")||customElements.define("petkit-feeder-card-editor",Me),window.customCards=window.customCards||[],window.customCards.push({type:"petkit-feeder-card",name:"小佩喂食器",description:"显示小佩喂食器状态、喂食计划和历史记录",preview:!0,documentationURL:"https://github.com/ningjx/petkit-feeder-card"});export{Ue as PetkitFeederCard,Me as PetkitFeederCardEditor};
